...
const conversationContexts: Map<string, any> = new Map();
...
app.post("/twiml", async (req, res) => {
  ...
  try {
    ...
    // Try to get user context from backend if user_id is provided
    ...
    if (userId) {
      try {
        ...
        if (response.ok) {
          ...
          userContext = data;
          conversationContexts.set(callId, userContext);
          log.app.info(`[${callId}] Loaded user context for user ${userId}`);
        }
      } catch (error) {
        log.app.warn(`[${callId}] Could not fetch user context: ${error}`);
      }
    }
  ...
  xaiWs.on('message', (data: Buffer) => {
    try {
      ...
      if (message.type === 'response.output_audio.delta' && message.delta) {
        ...
        // Get user context for personalized instructions
        const userContext = conversationContexts.get(callId);
        let instructions = bot.instructions;
        
        if (userContext) {
          // Generate personalized instructions
          try {
            const personalizedInstructions = await bot.getInstructions(callId, userContext);
            instructions = personalizedInstructions;
            log.app.info(`[${callId}] Using personalized instructions based on user context`);
          } catch (error) {
            log.app.warn(`[${callId}] Could not get personalized instructions: ${error}`);
          }
        }
        ...
        // Store transcript for backend when available
        // This will be handled when we get the transcript
      } else if (message.type === 'input_audio_buffer.committed') {
        log.app.info(`[${callId}] Audio buffer committed (${message.item_id || 'no item_id'})`);
        
        // Store item_id for transcript matching
        const userContext = conversationContexts.get(callId);
        if (userContext && message.item_id) {
          if (!userContext.pending_transcripts) {
            userContext.pending_transcripts = new Map();
          }
          userContext.pending_transcripts.set(message.item_id, {
            timestamp: new Date().toISOString()
          });
        }
      } else if (message.type === 'input_audio_buffer.transcript.delta' || 
                 message.type === 'input_audio_buffer.transcript.done') {
        // Handle user transcript
        const userContext = conversationContexts.get(callId);
        if (userContext && message.item_id) {
          const transcript = message.transcript || message.delta || '';
          
          if (transcript) {
            // Store transcript
            if (!userContext.transcripts) {
              userContext.transcripts = [];
            }
            
            // Find or create transcript entry
            let transcriptEntry = userContext.transcripts.find((t: any) => t.item_id === message.item_id);
            if (!transcriptEntry) {
              transcriptEntry = {
              ...
              userContext.transcripts.push(transcriptEntry);
            }
            
            transcriptEntry.text += transcript;
            
            // If transcript is done, send to backend
            if (message.type === 'input_audio_buffer.transcript.done') {
              log.app.info(`[${callId}] 📝 User transcript: "${transcriptEntry.text}"`);
              
              // Send to backend for memory storage
              if (userContext.user_id) {
                try {
                  await fetch(`${BACKEND_URL}/api/conversations/${userContext.conversation_id || callId}/message`, {
                    method: 'POST',
                  ...
                  log.app.warn(`[${callId}] Could not send transcript to backend: ${error}`);
                }
              }
            }
          }
...
app.post("/api/calls/initiate", async (req, res) => {
  try {
    const { user_id, username, phone_number, context } = req.body;
    ...
    const backendResponse = await fetch(`${BACKEND_URL}/api/calls/initiate`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    const callId = backendData.conversation_id || generateSecureId('call');
    conversationContexts.set(callId, {
      user_id: user_id,
      username: username,
      conversation_id: backendData.conversation_id,
      context: context || {},
      initial_message: backendData.initial_message
    });
    
    // Here you would initiate the Twilio call
    // For now, return the conversation info
    res.json({
      conversation_id: backendData.conversation_id,
      call_id: callId,
      phone_number: phone_number,
      initial_message: backendData.initial_message,
      status: "ready",
      note: "Use Twilio API to initiate call with this conversation_id"
    });
    
  } catch (error: any) {
    log.app.error(`Error initiating call: ${error}`);
